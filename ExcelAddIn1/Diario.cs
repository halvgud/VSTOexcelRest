...
namespace ExcelAddIn1
{
    public partial class Diario : Form
    {
        ...
        private BindingList<Reporte.RespuestaCocina.RepoDiario> _listaOtrosDiariosHoy;
        ...
        public Diario()
        {
            ...
            _listaOtrosDiariosHoy=new BindingList<Reporte.RespuestaCocina.RepoDiario>();
            InitializeComponent();
        }
        public class Final
        {
            ...
            Opcion.EjecucionAsync(Data.Reporte.Yesterday, jsonResult =>
                {
                    BeginInvoke((MethodInvoker)(() =>
                    {
                        ...
                        dgvInventarioPlatillos.DefaultCellStyle.Font = new Font("Arial", 10, FontStyle.Bold);
        ...
        private void btCargarDiarios_Click(object sender, EventArgs e)
        {
            ...
            if (jsonResult != null)
            {
                BeginInvoke((MethodInvoker)(() =>
                {
                    ...
                    {
                        case HttpStatusCode.OK:
                           var filaprimera = new BindingList<Reporte.RespuestaCocina.RepoDiario>();
                            filaprimera.AddNew();
                           var list=new List<Reporte.RespuestaCocina.RepoDiario>(Opcion.JsonaListaGenerica<Reporte.RespuestaCocina.RepoDiario>(jsonResult));
                            ...
                            foreach (Reporte.RespuestaCocina.RepoDiario cc in (nuevalis))
                            {
                             filaprimera.Add(cc);     
                            }
                            ...
                            filaprimera.AddNew();  
                            dgvDiarios.DataSource = filaprimera;
                          
                            
                            for (int a = 0; a < dgvDiarios.RowCount-1; a++)
                            {   
                                if (dgvDiarios.Rows[a].Cells["Clave"].Value == null ||
                                    dgvDiarios.Rows[a].Cells["Platillo"].Value == null)
                                {
                                    dgvDiarios.Rows[a].ReadOnly = true;
                                    dgvDiarios.Rows[a].Cells["Platillo"].Value = "MENU DEL DIA";
                                    dgvDiarios.Rows[a].DefaultCellStyle.BackColor = Color.Black;
                                    dgvDiarios.Rows[a].Cells["Platillo"].Style.ForeColor = Color.Red;
                                    dgvDiarios.Rows[a].Cells["Platillo"].Style.Font = new Font("Arial", 14, FontStyle.Bold);
                                }
                            }
                            dgvDiarios.CurrentRow.ReadOnly = true;
                            dgvDiarios.CurrentRow.Cells["Platillo"].Value = "APROVECHAMIENTO DE SUCURSAL";
                            dgvDiarios.CurrentRow.DefaultCellStyle.BackColor = Color.Black;
                            dgvDiarios.CurrentRow.Cells["Platillo"].Style.ForeColor = Color.Red;
                            dgvDiarios.CurrentRow.Cells["Platillo"].Style.Font = new Font("Arial", 14, FontStyle.Bold);
                            dgvDiarios.Rows[dgvDiarios.RowCount - 2].ReadOnly = true;
                            dgvDiarios.Rows[dgvDiarios.RowCount - 2].Cells["Platillo"].Value = "OTROS PRODUCTOS A LA VENTA";
                            dgvDiarios.Rows[dgvDiarios.RowCount - 2].DefaultCellStyle.BackColor = Color.Black;
                            dgvDiarios.Rows[dgvDiarios.RowCount - 2].Cells["Platillo"].Style.ForeColor = Color.Red;
                            dgvDiarios.Rows[dgvDiarios.RowCount - 2].Cells["Platillo"].Style.Font = new Font("Arial", 14, FontStyle.Bold);

                            var nuevo = dgvDiarios.DataSource as BindingList<Reporte.RespuestaCocina.RepoDiario>;
                            foreach (Reporte.RespuestaCocina.RepoDiario dd in (_listaOtrosDiariosHoy))
                            {
                                if (nuevo != null)
                                {
                                    nuevo.Add(dd);
        ...
        private void btGuardarDiarios_Click(object sender, EventArgs e)
        {
            ...
            var row = dgvDiarios.CurrentCell.RowIndex;
            const int estado =1;
            var listaDiariosMa = new List<Receta.Savedaily>();
                var id = Convert.ToInt32(dgvDiarios.Rows[row].Cells["EstadoInventarioId"].Value);
                    if (id == 0)
                    {
                        if (dgvDiarios.Rows[row].Cells["Clave"].Value != null)
                        {
                            listaDiariosMa.Add(new Receta.Savedaily
                            {
                                ArtId = dgvDiarios.Rows[row].Cells["ArtId"].Value.ToString(),
                                Cantidad = 0,
                                Clave = dgvDiarios.Rows[row].Cells["Clave"].Value.ToString(),
                                Platillo = dgvDiarios.Rows[row].Cells["Platillo"].Value.ToString(),
                                CantidadCocina = Convert.ToDouble(dgvDiarios.Rows[row].Cells["CR"].Value.ToString()),
                                CantidadElaborar = Convert.ToDouble(dgvDiarios.Rows[row].Cells["CE"].Value),
                                Fecha = fechaactual,
                                EstadoInventarioId = Convert.ToInt32(dgvDiarios.Rows[row].Cells["EstadoInventarioId"].Value),  
                                EstadoDescripcionId = 5,
                                Status = estado.ToString(),
                                Observacion=""
                            });
                    ...
                    if (dgvDiarios.Rows[row].Cells["Clave"].Value != null)
                    {
                        var listaActualiza = new Receta.Savedaily
                        {
                            ArtId = dgvDiarios.Rows[row].Cells["ArtId"].Value.ToString(),
                            Cantidad = 0,
                            Clave = dgvDiarios.Rows[row].Cells["Clave"].Value.ToString(),
                        ...
                        if (Convert.ToInt32(dgvDiarios.Rows[row].Cells["Existencia"].Value) > 0)
                        {
                    if (dgvDiarios.Rows[row].Cells["Clave"].Value != null)
                    {
                        var clave = dgvDiarios.Rows[row].Cells["Clave"].Value.ToString();
        ...
        private void DiseñoDgvDiario()
        {
            ...
            dgvDiarios.DefaultCellStyle.Font = new Font("Arial", 10, FontStyle.Bold);
        ...
        private void CarganewDiario()
        {
                                ...
                                for (var i = 0; i < dgvDiarios.RowCount; i++)
                                {
                                    ...
                                    for (var j = 0; j < ListDiarioDs.Count; j++)
                                    {
                                        ...
                                        if (x == y)
                                        {
                                            ...
                                            dgvDiarios.Rows[i].Cells["EstadoInventarioId"].Value = ListDiarioDs[j].EstadoInventarioId;
                                        }
                                    }
                                }
                                break;
            ...
            Opcion.EjecucionAsync(x =>
            {
                var times = new Reporte.General
                {
                    FechaFin = Convert.ToDateTime(fecha1R)
                };
                Data.Reporte.MostrarEstado(x, times);
            }, z =>
            {
                            ...
                            case HttpStatusCode.OK:
                                ...
                                for (var i = 0; i < dgvDiarios.RowCount; i++)
                                {
                                    ...
                                    for (var j = 0; j < ListDiarioDs.Count; j++)
                                    {
                                        ...
                                        {
                                            Xyz = i;    
                                            dgvDiarios.Rows[i].Cells["Existencia"].Value = ListDiarioDs[j].Existencia;
                                        }
                                    }
                                }
                                break;
        ...
        private void OtrosPlatillosMenuDia()
        {
            var fechainicior = DateTime.Now.AddDays(-30).ToString("yyyy-MM-dd");
            var fechafinalr = DateTime.Now.ToString("yyyy-MM-dd");
            var fechas = new Cocina.ReporteDiarioCocina.FechasReporte
            ...
            Opcion.EjecucionAsync(x =>
            {
                Data.Reporte.RepoOtrosPlatillosHoy(x, fechas);
            }, w =>
            {
                if (w != null)
                {
        ...
        private void dgvDiarios_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
                ...
                dgvDiarios.Rows[rowIndex].Cells["Platillo"].Value.ToString() != "" && dgvDiarios.Rows[rowIndex].Cells["Clave"].Value == null)
            {
                ...
                }, z =>
                {
                    ...
                    { if(dgvDiarios.Rows[e.RowIndex].Cells["Platillo"].Value.ToString()!="APROVECHAMIENTO DE SUCURSAL" || 
                        dgvDiarios.Rows[e.RowIndex].Cells["Platillo"].Value.ToString()!= "MENU DEL DIA" ||
                        dgvDiarios.Rows[e.RowIndex].Cells["Platillo"].Value.ToString() != "OTROS PRODUCTOS A LA VENTA")return;
                        MessageBox.Show(@"El nombre del platillo no es el correcto, favor de verificar");
                    }
                });
